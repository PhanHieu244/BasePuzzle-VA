...
public class PGameProgressManager : MonoBehaviour 
{
	/// <summary>
	...
	{
		string oldProgrss = PlayerPrefs.GetString ("GameProgress" + PGameController.gameMode.ToString (), string.Empty);
	...
	public void SaveGame()
	{
		...
		if (xDocProgress != null) {
			...
			GameModeElement.Value = PGameController.gameMode.ToString ();
			#endregion
			...
			for (int rowID = 0; rowID < PGameBoardGenerator.Instance.TotalRows; rowID++) {
				string row = "";
				for (int columnID = 0; columnID < PGameBoardGenerator.Instance.TotalColumns; columnID++) {
					row += (PGamePlay.Instance.blockGrid [index].blockID + ",");
					index++;
				}
			...
			#region placing blocks
			xDocProgress.Root.Element ("ShapeInfo").Value = PBlockShapeSpawner.Instance.GetAllOnBoardShapeNames ();
			#endregion


			#region score
			xDocProgress.Root.Element("Score").Value = PScoreManager.Instance.GetScore().ToString();
			#endregion

			#region total rescue done
			xDocProgress.Root.Element("TotalRescueDone").Value = PGamePlay.Instance.TotalRescueDone.ToString();
			#endregion

			#region total free rescue done
			xDocProgress.Root.Element("TotalFreeRescueDone").Value = PGamePlay.Instance.TotalFreeRescueDone.ToString();
			#endregion
			...
			if(PGameController.gameMode == GameMode.BLAST || PGameController.gameMode == GameMode.CHALLENGE)
			{
				...
				List<PBlock> bombBlocks = PGamePlay.Instance.blockGrid.FindAll (o => o.isBomb);
			...
			if(PGameController.gameMode == GameMode.TIMED || PGameController.gameMode == GameMode.CHALLENGE)
			{
				xDocProgress.Root.Element("RemainingTime").Value = PGamePlay.Instance.timeSlider.GetRemainingTime().ToString();
			}
			#endregion

			#region moves count
			xDocProgress.Root.Element("MovesCount").Value = PGamePlay.Instance.MoveCount.ToString();
			#endregion

			#region save progress
			PlayerPrefs.SetString("GameProgress" + PGameController.gameMode.ToString (),xDocProgress.ToString());
			#endregion
		}
	}
	...
	public void ClearProgress(){
		PlayerPrefs.DeleteKey ("GameProgress" + PGameController.gameMode.ToString ());
	}
